...
int serial_send(int serial_fd)
{
	...
	uint8_t sys_type = 1; //fixed wing
	uint8_t autopilot_type = 1; //pixhawk typ
	uint8_t sys_id = 250;
	uint8_t comp_id = 1;
	uint8_t target_sys = 1;
	uint8_t base_mode = 209;
	uint8_t sys_status = 3;
	uint16_t servo = 1900;
	//int8_t base_mode = 0;
	/*
	mavlink_message_t manmsg;
	mavlink_msg_manual_control_pack(sys_id, comp_id, &manmsg, target_sys, 500, 500, 500, 500, 1111);
	uint8_t lenman = mavlink_msg_to_send_buffer(buf2, &manmsg);	
	write(fd, buf2, lenman);
	printf("Sent man msg\n");
	tcdrain(fd); 
	*/
	while (1)
	{
		...
		mavlink_message_t rcmsg;
		
		//send a heartbeat per second
		if (loopcounter % 30 == 0 && hb_bool == true) {
			//ssy id, from comp id, msg, type of mav, autopilot type, base mode, custom mode, status	
			mavlink_msg_heartbeat_pack(sys_id, comp_id, &hbmsg, sys_type, autopilot_type, 0, 0, sys_status);
			uint8_t lenhb = mavlink_msg_to_send_buffer(buf2, &hbmsg);
			write(fd, buf2, lenhb);
			tcdrain(fd);
			printf("Sent heartbeat message\n");
		}
		if (loopcounter % 10 == 5 && mode_bool == true){
			//sys id, from comp id, msg, tagret sys, new base mode, custom mode			
			mavlink_msg_set_mode_pack(sys_id, comp_id, &modemsg, target_sys, 1, 4); 
			uint8_t lenmode = mavlink_msg_to_send_buffer(buf2, &modemsg);
			write(fd, buf2, lenmode);
			tcdrain(fd);
			printf("Sent mode = %d \n", modecounter);
			modecounter++;
			if(modecounter == 10) modecounter = 0;	
		}
		if (loopcounter % 2 == 0 && rc_bool == true){	
			//sys id, comp type, message, target system, target component, values....	
			mavlink_msg_rc_channels_override_pack(sys_id,comp_id,&rcmsg,target_sys,1,servo,servo,servo,servo,0,0,0,0);
			uint8_t len = mavlink_msg_to_send_buffer(buf2, &rcmsg);
			write(fd, buf2, len);
			tcdrain(fd);
			printf("wrote rc= %d to sys id = %d comp id = %d \n", servo, target_sys, 1);
		}
		usleep(100000);
		loopcounter++;
	}
...
int main(int argc, char **argv) {
	...
	int baudrate = 57600;
	const char *commandline_usage = "\tusage: %s -d <devicename> -b <baudrate> -m <true> -hb <true> -rc <true [-v/--verbose] [--debug]\n\t\tdefault: -d %s -b %i -m false -hb false -rc false\n";
	...
	for (i = 1; i < argc; i++) { /* argv[0] is "mavlink" */
		...
		if (strcmp(argv[i], "-m") == 0) {
			if (argc > i + 1){				
				if (strcmp(argv[i+1], "true") == 0) mode_bool = true;
			} else {
				printf(commandline_usage, argv[0], uart_name, baudrate);
				return 0;
			}
		...
		if (strcmp(argv[i], "-hb") == 0) {
			if (argc > i + 1){				
				if (strcmp(argv[i+1], "true") == 0) hb_bool = true;
			} else {
				printf(commandline_usage, argv[0], uart_name, baudrate);
				return 0;
			}
		...
		if (strcmp(argv[i], "-rc") == 0) {
			if (argc > i + 1){				
				if (strcmp(argv[i+1], "true") == 0) rc_bool = true;
			} else {
				printf(commandline_usage, argv[0], uart_name, baudrate);
				return 0;
			}
	...
	if (!hb_bool && !rc_bool && !mode_bool){
		printf("Whatchu doing? include -h true or something mayn. \n");
		exit(EXIT_FAILURE);
	}
